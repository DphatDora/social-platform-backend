...
func SetupRoutes(db *gorm.DB, redisClient *redis.Client, conf *config.Config) *gin.Engine {
	...
	authService := service.NewAuthService(userRepo, verificationRepo, passwordResetRepo, refreshTokenRepo, botTaskRepo, communityModeratorRepo, notificationSettingRepo, botTaskService, redisClient)
	userService := service.NewUserService(userRepo, communityRepo, communityModeratorRepo, subscriptionRepo, userSavedPostRepo, postRepo, refreshTokenRepo, botTaskService, redisClient)
